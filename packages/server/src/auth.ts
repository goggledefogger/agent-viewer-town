...
export function validateToken(token?: string): boolean {
  ...
  // Use SHA-256 to hash the tokens first. This guarantees both buffers are the same length (32 bytes),
  // which is required by timingSafeEqual, and prevents leaking the token length.
