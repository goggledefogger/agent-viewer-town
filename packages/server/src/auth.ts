...
export function validateToken(token?: string): boolean {
  ...
  try {
    const hashedGivenToken = crypto.createHash('sha256').update(token).digest();
    const hashedServerToken = crypto.createHash('sha256').update(serverToken).digest();
    return crypto.timingSafeEqual(hashedGivenToken, hashedServerToken);
  } catch (err) {
    return false;
  }
}
