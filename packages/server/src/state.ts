import type { TeamState, AgentState, TaskState, MessageState, SessionInfo, SessionListEntry, GroupedSessionsList, ProjectGroup, BranchGroup, WSMessage } from '@agent-viewer/shared';
...
export class StateManager {
  ...
  getGroupedSessionsList(activeSessionId?: string): GroupedSessionsList {
    const effectiveActiveId = activeSessionId ?? this.state.session?.sessionId;
    const flatSessions = this.getSessionsList(effectiveActiveId);
    ...
    for (const [projectKey, { name, path, branches }] of projectMap) {
      ...
      for (const [branchName, sessions] of branches) {
        // Sort sessions within branch: active first, then waiting, then remaining
        sessions.sort((a, b) => {
          const aActive = a.sessionId === effectiveActiveId ? 1 : 0;
          const bActive = b.sessionId === effectiveActiveId ? 1 : 0;
      ...
      branchGroups.sort((a, b) => {
        ...
        const bHasActive = b.sessions.some((s) => s.sessionId === effectiveActiveId);
