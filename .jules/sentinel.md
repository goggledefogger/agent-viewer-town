...
## 2025-06-07 - Express cors Middleware Fallback Requirement
**Vulnerability:** The application was using the `cors` package to reject unauthorized cross-origin requests by returning `callback(null, false)` in the origin function. However, this only omits CORS headers (`Access-Control-Allow-Origin`, etc) and allows the request to be processed by the server (returning a 200). Furthermore, the application's origin check allowed the literal string `'null'`, which can be exploited by sandboxed iframes.
**Learning:** Returning `false` in the `cors` package's origin callback does not block the request. The request is still processed, and while browsers may block reading the response, state-changing requests (like POST or GET with side effects) still execute. Additionally, origin validation logic must explicitly block the origin `'null'` which may otherwise pass `new URL(origin)` logic check but is typically a bypass vector for local testing and iframe sandboxing.
**Prevention:** Always add an explicit fallback middleware *after* `app.use(cors())` that manually checks `req.headers.origin` and explicitly returns a `403 Forbidden` if the origin is unauthorized. Additionally, origin validation routines must check for and explicitly block the string `'null'`.
