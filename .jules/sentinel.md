...
## 2026-05-14 - CORS Fallback and `null` Origin Exploitation
**Vulnerability:** The server used the `cors` package configured to return `false` in the origin callback for disallowed origins, which only omitted CORS headers rather than actively blocking requests. Additionally, the string `'null'` (sent by sandboxed iframes) was not explicitly blocked, potentially allowing unauthorized cross-origin access.
**Learning:** Returning `false` in the `cors` middleware origin callback prevents cross-origin data reads in the browser, but doesn't stop the request from reaching the server endpoint (e.g., executing side effects). Furthermore, some environments (like `URL` constructors) may parse or accept `'null'` if not explicitly rejected.
**Prevention:** Always implement an explicit fallback middleware to verify `req.headers.origin` and return a `403 Forbidden` status for unauthorized requests. Additionally, explicitly reject the string `'null'` in origin validation logic to prevent bypasses via sandboxed iframes.
