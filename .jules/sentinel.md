...
## 2026-06-10 - Predictable Temporary File Symlink Vulnerability
**Vulnerability:** Writing predictable temporary files to world-writable directories like `/tmp/agent-viewer-touchbar.json` exposes the application to symlink attacks, allowing arbitrary file overwrites.
**Learning:** Shared directories like `/tmp` are accessible by all users on a system. A malicious user could create a symbolic link at the expected file location pointing to a sensitive file (e.g., `/etc/passwd`). When the application writes to the predictable path, it inadvertently overwrites the target file with application data.
**Prevention:** Always write application-specific files to user-owned directories (e.g., using `os.homedir()` or `$HOME`) or utilize dynamically generated temporary files via secure APIs like `mkstemp` or `mkdtemp`.
