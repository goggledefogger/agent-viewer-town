...
## 2025-03-01 - Explicit Fallback Middleware Needed When Omitting CORS Headers
**Vulnerability:** HTTP endpoints configured with `cors()` were returning `false` for unauthorized origins to silently drop CORS headers, but still processed the request through standard route handlers, allowing unauthorized cross-origin requests to execute logic and obtain a 200 OK status if the browser ignored the missing CORS headers or for non-browser clients.
**Learning:** When using the Express `cors` middleware, returning `callback(null, false)` merely prevents the appending of `Access-Control-Allow-Origin` headers. It does not actively reject or terminate the request with a `403 Forbidden` status. The request still falls through to subsequent middleware and route logic.
**Prevention:** Always implement an explicit fallback middleware immediately following `app.use(cors(...))` that checks `req.headers.origin` against the allowed list and actively returns a `403 Forbidden` response to terminate the request for unauthorized origins.
