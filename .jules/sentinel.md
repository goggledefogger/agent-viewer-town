...
## 2024-06-06 - Missing explicit CORS fallback middleware
**Vulnerability:** The Express CORS configuration relies solely on the `cors` middleware, but passing `callback(null, false)` merely omits CORS headers without blocking the request (which is the default behavior of the `cors` package). Thus, standard API routes may still process requests from unauthorized origins unless an explicit fallback block is provided.
**Learning:** The `cors` package does not automatically send a 403 response when `origin: (origin, cb) => cb(null, false)` is used; it just doesn't set CORS headers. We must add custom middleware to explicitly reject disallowed origins with a 403 Forbidden.
**Prevention:** In environments using `cors` dynamic origin resolution, explicitly add a middleware that checks the origin and returns a 403.
