...
## 2026-06-29 - [Server CSRF Bypass via CORS Middleware Omission]
**Vulnerability:** The Express `cors` middleware configured with `callback(null, false)` for invalid origins successfully omits CORS headers but DOES NOT terminate the request cycle. State-modifying endpoints like `/api/hook` remained accessible to malicious origins (CSRF bypass) because the request was still processed by subsequent handlers.
**Learning:** Returning `false` in a custom `cors` origin function is insufficient for request blocking in Express. It only suppresses standard CORS headers, which prevents browsers from reading responses, but doesn't prevent side-effects from execution (like state changes or database writes).
**Prevention:** Always include a dedicated fallback middleware immediately following the `cors` middleware that explicitly re-evaluates the `Origin` header and responds with a `403 Forbidden` status to block the execution flow.
