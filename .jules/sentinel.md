...
## 2025-02-18 - [CSRF Vulnerability via Express CORS bypass]
**Vulnerability:** The Express `cors` middleware configured with `callback(null, false)` successfully omits CORS headers for unauthorized origins, but it does NOT terminate the request. Consequently, unauthorized requests (e.g., cross-origin POST requests without preflights) bypass CORS but still execute the target route handler, causing CSRF state modifications.
**Learning:** Returning `false` in the CORS origin callback is insufficient to prevent request execution in Express. Additionally, sandboxed iframes can bypass standard origin checks by sending `'null'` as the Origin header.
**Prevention:** Always add an explicit fallback middleware *after* `cors()` to validate `req.headers.origin` and proactively return a 403 Forbidden status for rejected origins, ensuring subsequent route handlers are not executed. Furthermore, explicitly block `'null'` string origins in origin validation logic.
