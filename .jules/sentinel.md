...
## 2026-02-14 - CORS Middleware Gap Exposing State to CSRF Side-Effects
**Vulnerability:** The Express `cors` middleware, when returning `callback(null, false)` for invalid origins, only omitted the CORS headers rather than terminating the request. This allowed cross-origin POST requests to proceed to state-modifying endpoints (like `/api/hook`), causing unintended state mutations despite the browser ultimately rejecting the response. Additionally, 'null' origins were not explicitly blocked, which can bypass some origin validation checks.
**Learning:** Returning `false` in the `cors` middleware callback does not intrinsically prevent the request from reaching the route handler in Express. A dedicated middleware is necessary to enforce a hard rejection (e.g., HTTP 403) for disallowed origins. Furthermore, sandboxed iframes and certain redirects use the origin string `'null'`, which must be explicitly handled in origin validation logic.
**Prevention:** Implement a fallback middleware immediately following `cors()` to explicitly validate `req.headers.origin` and respond with `403 Forbidden` if it's invalid. Additionally, explicitly check for and reject the `'null'` string in custom origin validation functions like `isAllowedOrigin`.
