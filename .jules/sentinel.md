...
## 2025-05-17 - Bypassing CORS Restrictions in the 'cors' Package
**Vulnerability:** The local development server (`packages/server`) used the `cors` package but only returned `false` in the origin validation callback. This prevented CORS headers from being sent but did not block the underlying requests from unauthorized origins from executing, allowing Cross-Origin Resource Sharing bypass.
**Learning:** Returning `false` in the `cors` origin validation function only causes `cors` to omit standard CORS headers (e.g. `Access-Control-Allow-Origin`). The request is still processed by the endpoint and will return a successful status code.
**Prevention:** Implement an explicit fallback middleware after `cors` that actively checks `req.headers.origin` and explicitly returns a `403 Forbidden` status for invalid origins. Additionally, ensure the literal string `"null"` is blocked when performing origin checks.
