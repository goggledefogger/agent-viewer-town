...
## 2026-05-21 - Express CORS Middleware Bypass Fix
**Vulnerability:** The default Express `cors()` middleware implementation was returning `callback(null, false)` for unauthorized origins. While this successfully omitted CORS headers, it allowed the request to proceed to the routing layer, potentially exposing state-changing endpoints to Cross-Site Request Forgery (CSRF). Additionally, the origin validation stringently checked against `URL` objects but was vulnerable to the `'null'` origin bypass (used by sandboxed iframes).
**Learning:** `cors()` merely handles CORS header omission/inclusion. When `callback(null, false)` is used instead of returning an error, an explicit fallback middleware checking `req.headers.origin` must be utilized to actively return a 403 Forbidden status for unauthorized requests. Additionally, `'null'` origin string must be explicitly checked.
**Prevention:** Always pair `cors()` with a fallback blocking middleware if the intent is to protect against unauthorized cross-origin execution, and ensure validation checks strictly handle edge cases like the `'null'` origin.
