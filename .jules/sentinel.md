...
## 2025-02-14 - Authentication Token Timing Attack Vulnerability
**Vulnerability:** The server authentication mechanism in `packages/server/src/auth.ts` used strict equality (`===`) to compare the provided token with the server's configured `AUTH_TOKEN`. This made the endpoint susceptible to timing attacks, where an attacker could potentially infer the correct token by measuring the time it takes for the server to reject incorrect tokens character by character.
**Learning:** Standard string comparison operators in JavaScript (`===`, `==`) fail fast as soon as they encounter a mismatching character. The longer the comparison takes, the more characters matched. In a security context (like validating secrets, passwords, or authentication tokens), this variable execution time leaks information about the secret.
**Prevention:** When comparing secrets like authentication tokens, always use a constant-time comparison algorithm. In Node.js, this is achieved using `crypto.timingSafeEqual`. Furthermore, to ensure the input buffers have the exact same length (a requirement for `timingSafeEqual`) and to prevent length-based timing leaks, hash both the provided token and the expected secret (e.g., using SHA-256) before passing them to `timingSafeEqual`.
