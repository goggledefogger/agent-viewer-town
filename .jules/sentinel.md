...
## 2026-05-16 - CORS Bypass via Missing 403 Response and Null Origins
**Vulnerability:** The application was vulnerable to Cross-Site Request Forgery (CSRF) for simple requests and iframe-based bypasses because the Express `cors` middleware, when rejecting a cross-origin request, only omitted the CORS headers rather than terminating the request. Additionally, sandboxed iframes sending an `Origin: null` header were not explicitly blocked.
**Learning:** Using `callback(null, false)` in the Express `cors` middleware does not send a `403 Forbidden` response; it simply causes the response to omit CORS headers. The request continues to be processed by subsequent middleware/routes. This allows cross-origin requests that don't trigger preflights (like simple POST forms) to successfully execute state-changing actions.
**Prevention:** Always pair `cors` middleware configuration with a fallback middleware that explicitly checks `req.headers.origin` and terminates unauthorized requests with `res.status(403)`. Furthermore, explicitly check for and block `origin === 'null'` to prevent attacks originating from local files or sandboxed iframes.
