...
## 2026-02-14 - Predictable Files in World-Writable Directory (/tmp)
**Vulnerability:** Predictable file and directory paths written to `/tmp` expose the application to symlink attacks. An attacker can pre-create a symlink at the predictable location pointing to a sensitive file, causing the application to overwrite it with its own output when executed.
**Learning:** Hardcoded `/tmp` paths like `/tmp/agent-viewer-touchbar.json` and `/tmp/touchbar-pomodoro-state` in application logic or scripts are predictable and dangerous.
**Prevention:** Avoid writing predictable files to world-writable directories. Use user-owned directories like `$HOME` or `os.homedir()` for application state, or dynamically generated temporary files/directories using `mktemp` or `fs.mkstemp` when temporary storage is needed.
