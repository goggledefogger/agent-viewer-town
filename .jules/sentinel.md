...
## 2025-02-18 - [Fix CORS False Negatives and Sandboxed Iframe Bypass]
**Vulnerability:** The application's origin validation (`isAllowedOrigin`) did not explicitly block the `'null'` string, allowing sandboxed iframes to bypass the origin check. Furthermore, the `cors` middleware, when configured with `callback(null, false)` for rejected origins, simply omitted the `Access-Control-Allow-Origin` headers. While this prevents browsers from reading the response, it still allows simple cross-origin requests to be executed on the server, posing a risk of Cross-Site Request Forgery (CSRF).
**Learning:** Returning `false` in the `cors` package's origin callback only omits CORS headers; it does not block the request execution. Relying solely on standard CORS middleware without explicit rejection allows state-changing requests to reach the server. Additionally, `new URL('null')` will throw a `TypeError`, but explicit checking for `'null'` clarifies intent and prevents unexpected bypasses.
**Prevention:** Implement an explicit fallback middleware immediately following the `cors` middleware to check `req.headers.origin` and proactively return a `403 Forbidden` status for unauthorized origins. Ensure origin validation logic explicitly rejects `'null'`.
