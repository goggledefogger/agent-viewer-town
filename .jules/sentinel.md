...
## 2026-06-21 - [CORS Origin Fallback Hardening]
**Vulnerability:** The CORS configuration omitted the Access-Control-Allow-Origin header for unauthorized requests but still processed the request, leaving state-changing endpoints vulnerable to CSRF and side-channel attacks. The application also lacked explicit handling for the 'null' origin string often used to bypass sandboxes.
**Learning:** Relying solely on the `cors` package to omit headers is insufficient for protecting the API against unauthorized access; the underlying Express route still executes unless specifically aborted with a 403 response.
**Prevention:** Always implement explicit origin validation middleware alongside the CORS package to forcibly return a 403 Forbidden status for requests originating from disallowed domains, and explicitly check for and reject 'null' origins.
