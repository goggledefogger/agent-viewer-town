...
## 2024-05-24 - Insecure Temporary File Usage
**Vulnerability:** Use of predictable files in the world-writable `/tmp` directory (`/tmp/agent-viewer-touchbar.json` and `/tmp/Launcher.applescript`). This allows for symlink attacks, arbitrary file overwrite, or unauthorized access.
**Learning:** Hardcoded `/tmp` paths shouldn't be used, especially since any local user can create a symlink with that name before the application does, pointing it to a sensitive file (like `~/.ssh/authorized_keys` or `/etc/passwd`). When the application writes to the tmp file, it overwrites the sensitive file.
**Prevention:** Use user-specific temporary directories like `os.homedir()` or `os.tmpdir()` combined with unique file generation, or place predictable files in user-owned directories such as `~/.agent-viewer-touchbar.json`.
