...
## 2025-03-01 - Express CORS Middleware Bypass for Simple Requests
**Vulnerability:** The Express `cors()` middleware defaults to omitting CORS headers when rejecting an origin, but it does NOT block the request from executing route handlers. This allows "simple requests" (like POSTing from an HTML form on a malicious site) to trigger state-changing API endpoints without explicit rejection.
**Learning:** Returning `callback(null, false)` in the `cors()` origin function only affects the CORS headers sent back, which stops browsers from reading the response. It does not stop the server from processing the request. This leaves the server open to Cross-Site Request Forgery (CSRF).
**Prevention:** Always pair `cors()` with an explicit fallback middleware that checks `req.headers.origin` against the allowed list and forcefully responds with a `403 Forbidden` to halt execution.
