...
**Vulnerability:** The `validateToken` function used strict equality (`===`) to compare authentication tokens. This could leak the token character-by-character through a timing attack.
**Learning:** Even internal security functions (like `validateToken` checking an environment `AUTH_TOKEN`) need constant-time string comparisons. Node's `crypto.timingSafeEqual` is ideal for this, but requires `ArrayBuffer` or `Buffer` objects of the exact same length.
**Prevention:** Always hash authentication tokens using algorithms like SHA-256 before comparing them using `crypto.timingSafeEqual`. This ensures both strings are converted to `Buffer` objects of identical length, allowing secure comparison and preventing timing attacks.
