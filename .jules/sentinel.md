...
## 2024-05-28 - Windows Executable Path Hijacking Mitigation Fix
**Vulnerability:** The codebase was trying to prevent current-directory executable hijacking on Windows by setting `NoDefaultCurrentDirectoryInExePath: '1'` in the `env` options of `child_process.execFile` calls (e.g. in `touchbar.ts`). This is ineffective.
**Learning:** Node.js resolves executable paths in the parent process via `libuv` before spawning the child. Setting `NoDefaultCurrentDirectoryInExePath` in the child's environment variables has no effect on the path resolution.
**Prevention:** To successfully prevent executable hijacking on Windows when using `child_process.execFile` without explicit absolute paths, `process.env.NoDefaultCurrentDirectoryInExePath = '1'` must be set globally at the application entry point (in the parent process).
