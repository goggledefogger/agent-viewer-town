...
## 2023-10-27 - CORS Misconfiguration Bypass
**Vulnerability:** Express `cors` middleware configured with `callback(null, false)` for invalid origins only omits CORS headers; it does not block the request. This allows simple cross-origin POST requests to reach the server and process actions.
**Learning:** Returning false in the cors library's origin callback does not result in a 403 Forbidden. It simply delegates the handling of the missing headers to the browser, but the request still hits the server.
**Prevention:** To forcefully block unauthorized origins, an explicit middleware must be added that checks `req.headers.origin` and returns a 403 status code when the origin is invalid.
