...
**Vulnerability:** Comparing authentication tokens using strict equality (`===`) allows an attacker to perform a timing attack. Since the comparison stops at the first mismatched character, the time taken to validate the token leaks information about its contents and length, making brute-force attacks feasible.
**Learning:** Using `===` for cryptographic or sensitive comparisons is unsafe due to short-circuiting behavior. Node.js provides `crypto.timingSafeEqual` specifically to prevent timing attacks by ensuring the comparison takes a constant amount of time regardless of whether characters match.
**Prevention:** Always use `crypto.timingSafeEqual` for comparing secrets like authentication tokens. Additionally, since `timingSafeEqual` requires the inputs to be of the exact same length (and throws an error otherwise), hash both the provided and expected tokens (e.g., using SHA-256) before comparison. This standardizes the length and prevents length-extension attacks while maintaining constant-time comparison.
