...
## 2025-02-12 - Predictable Temporary Files Lead to Symlink Vulnerability
**Vulnerability:** Predictable temporary files were written to the world-writable `/tmp` directory. For instance, scripts and the `touchbar.ts` module used hardcoded paths like `/tmp/agent-viewer-touchbar.json` and `/tmp/touchbar-pomodoro-state.cmd`.
**Learning:** Writing to predictable paths in `/tmp` without using secure mechanisms like `mkstemp`/`mkdtemp` allows a local attacker to pre-create these files as symlinks. When the application runs, it will overwrite the target of the symlink with its own output, leading to arbitrary file overwrite or privilege escalation (depending on context).
**Prevention:** Always write predictable temporary files or application state files to user-owned directories (e.g., within `os.homedir()` or `XDG_RUNTIME_DIR`) rather than world-writable shared directories. If a shared directory must be used, utilize securely generated randomized filenames.
