...
## 2024-05-11 - [CORS Library Fallback Bypass]
**Vulnerability:** The `cors` library was configured to return `false` on invalid origins, but it just omitted CORS headers instead of actively blocking simple requests (e.g. standard form POST).
**Learning:** Returning `callback(null, false)` in the `cors` Express middleware's origin function omits CORS headers but does not stop request execution on the server. To forcefully block unauthorized origins, an explicit fallback middleware checking `req.headers.origin` must be used.
**Prevention:** Always implement an explicit 403 Forbidden fallback middleware when relying on `cors` for origin validation if unauthorized requests shouldn't be executed at all.
