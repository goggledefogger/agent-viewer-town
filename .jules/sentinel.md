...
## 2025-03-01 - Express CORS Null Origin Bypass
**Vulnerability:** The server used the Express `cors` middleware, which correctly omitted CORS headers for disallowed origins but did not block the request itself. Furthermore, it did not explicitly reject the `'null'` origin string, which can be sent by sandboxed iframes or local HTML files. This allowed potential CSRF or cross-origin bypasses.
**Learning:** Returning `callback(null, false)` in the `cors` package origin check does not stop the server from processing the request; it only prevents the browser from reading the response. An explicit fallback middleware is required to return a `403 Forbidden` status to forcefully block unauthorized origins.
**Prevention:** Always implement an explicit 403-fallback middleware after `cors()` to reject requests with disallowed `Origin` headers, and explicitly block `'null'` in custom origin validation logic.
