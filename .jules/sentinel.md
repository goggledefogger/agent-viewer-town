...
## 2026-02-14 - Timing Attack Vulnerability in Token Validation
**Vulnerability:** The token validation mechanism (`packages/server/src/auth.ts`) used strict equality (`===`) for comparing the provided authentication token against the configured server token. This approach can be vulnerable to timing attacks, allowing an attacker to deduce the token length or character by character over multiple requests by observing the time taken to reject the request.
**Learning:** Even internal or local services that rely on shared secrets must validate those secrets securely. Node.js's V8 engine optimizes string comparisons, resulting in early exits when characters don't match, which leaks timing information. While the risk is often lower for local networks, the defense-in-depth approach mandates securing authentication routines.
**Prevention:** Always utilize `crypto.timingSafeEqual` for sensitive string comparisons, such as authentication tokens. Furthermore, because `timingSafeEqual` requires the compared buffers to be of identical length (and will throw if they are not, leaking length info), both the expected token and the provided token must first be hashed using a secure algorithm (like SHA-256) to guarantee constant length and avoid leaking the token length itself.
