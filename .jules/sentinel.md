...
## 2024-05-01 - Sandboxed Iframe 'null' Origin Bypass
**Vulnerability:** The CORS configuration allowed any string not evaluating to a valid URL to throw an error, which was caught and returned false, omitting the CORS header. However, iframes with sandbox attributes can send 'null' as their origin, potentially bypassing loose validation.
**Learning:** `new URL('null')` is invalid, but relying implicitly on the throw to omit headers is less robust than explicitly handling and blocking known bypass strings like 'null', combined with an active 403 Forbidden rejection middleware.
**Prevention:** Always explicitly check for and block `origin === 'null'` in origin validation logic. Combine standard CORS middleware configurations with explicit fallback middleware that actively rejects unauthorized requests by returning a 403 Forbidden status, rather than just omitting the header.
