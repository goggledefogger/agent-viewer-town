...
## 2026-07-28 - Token Comparison Timing Attack Vulnerability
**Vulnerability:** The server authenticated requests by performing a direct strict equality check (`===`) between the provided token and the expected server token. This exposes the application to timing attacks, where an attacker could theoretically infer the content or length of the token by analyzing the time it takes the server to reject incorrect tokens.
**Learning:** Even simple token matching requires protection against side-channel timing attacks. Using `crypto.timingSafeEqual` is correct, but requires comparing buffers of the exact same length. Hashing both the expected and provided tokens (e.g., using SHA-256) before comparison guarantees equal buffer lengths regardless of input and prevents length inference.
**Prevention:** Always compare sensitive authentication tokens, secrets, or passwords using constant-time comparison algorithms like `crypto.timingSafeEqual`. To ensure equal length and hide the token's original length, hash both the expected and user-provided inputs prior to the constant-time comparison.
