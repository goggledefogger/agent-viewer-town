...
## 2026-02-14 - CORS "null" Origin Bypass and Missing Authorization Rejection
**Vulnerability:** Local server failed to reject requests where the `Origin` header was set to the string `"null"` (commonly sent by sandboxed iframes). In addition, while the Express `cors` middleware omitted CORS headers for disallowed origins, it did not actively block the request, leaving endpoints accessible to unauthorized Cross-Site Request Forgery (CSRF) or execution depending on the client.
**Learning:** Returning `callback(null, false)` in the Express `cors` middleware only affects CORS headers; it does not block the HTTP request pipeline. Furthermore, the string `"null"` must be explicitly rejected as an invalid `Origin` to mitigate sandbox iframe bypasses.
**Prevention:** Implement a fallback Express middleware immediately following `cors` to inspect `req.headers.origin` and explicitly return a `403 Forbidden` status code for unallowed origins. Always block the `"null"` origin string.
