...
**Vulnerability:** The server authentication mechanism in `packages/server/src/auth.ts` used strict equality (`===`) to compare the provided token against `process.env.AUTH_TOKEN`, which allows timing attacks to potentially deduce the token length or content.
**Learning:** String comparison operators (`===` or `==`) in JavaScript fail fast; they stop comparing as soon as a mismatch is found. This means comparing a string takes time proportional to the number of matching characters, which can be measured to guess the expected string character by character.
**Prevention:** When validating authentication tokens, always hash the tokens (e.g., using SHA-256) first and use `crypto.timingSafeEqual` for comparison to prevent leaking token length or content via timing attacks. Hashing first is important because `timingSafeEqual` requires the buffers to be of the same length, and hashing guarantees fixed-length buffers while also preventing length-based timing leaks.
