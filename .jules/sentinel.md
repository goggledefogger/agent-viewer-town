...
## 2024-05-01 - Predictable Temporary File Vulnerability (Symlink Attack)
**Vulnerability:** The Touch Bar integration scripts and server used a predictable, hardcoded temporary file path (`/tmp/agent-viewer-touchbar.json`) in a world-writable directory. This exposed the application to symlink attacks, where a malicious local user could create a symlink at that path pointing to a sensitive file (e.g., `/etc/passwd`), causing the application to overwrite it.
**Learning:** Writing predictable files to `/tmp` or other shared directories is inherently unsafe on multi-user systems.
**Prevention:** Always write user-specific status or configuration files to user-owned directories (e.g., using `os.homedir()` in Node.js or `$HOME` in shell scripts), or use dynamically generated, unpredictable temporary file names (e.g., `mktemp`).
