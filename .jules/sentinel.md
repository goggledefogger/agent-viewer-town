...
## 2025-06-27 - Forceful 403 Fallback for CORS Middleware
**Vulnerability:** Relying solely on the Express `cors` middleware often just omits headers for unauthorized origins, allowing the request side-effects to still execute.
**Learning:** The `cors` package doesn't enforce blocking natively by default when returning `false`; an explicit fallback middleware checking `req.headers.origin` is required to forcefully return a 403 status.
**Prevention:** Always implement an explicit 403-fallback middleware after `cors` to block unauthorized cross-origin requests, and ensure the string 'null' is explicitly denied in origin validation logic.
