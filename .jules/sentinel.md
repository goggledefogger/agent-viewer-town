...
## 2024-05-09 - [Fix Current-Directory Executable Hijacking on Windows]
**Vulnerability:** `execFile` or its promisified version (`execFileAsync`) were used without passing the `NoDefaultCurrentDirectoryInExePath: '1'` environment variable.
**Learning:** By default, when Node.js runs child processes on Windows, the system might search the current working directory for an executable if an explicit path isn't given. This opens up an executable hijacking vulnerability if an attacker drops a malicious executable in a workspace directory. Setting the environment variable on the child process's `env` object does not protect against this because path resolution happens in the parent process.
**Prevention:** Set `process.env.NoDefaultCurrentDirectoryInExePath = '1'` globally at the application entry point to prevent current-directory executable hijacking for all child process spawning across the entire application lifecycle.
