...
## 2026-03-05 - Insecure Predictable Temporary File Overwrite
**Vulnerability:** The server wrote Touch Bar status data to a hardcoded predictable temporary file `/tmp/agent-viewer-touchbar.json` using `writeFile`, which overwrites without enforcing file ownership or type.
**Learning:** This exposes the application to symlink attacks (arbitrary file overwrite) where a local attacker could pre-create a symlink at the predictable location pointing to a sensitive file (e.g. `~/.ssh/authorized_keys`), leading the server process to overwrite it with its payload.
**Prevention:** Always write to user-owned directories (e.g., `~/.agent-viewer-touchbar.json`) or use dynamically generated temporary files via `mkstemp`/`mkdtemp` rather than predictable locations in globally writable directories like `/tmp`.
