...
## 2024-06-18 - Predictable Temp File Vulnerability
**Vulnerability:** A predictable temp file path (`/tmp/agent-viewer-touchbar.json`) was used for Touch Bar integration state sharing, exposing the system to potential symlink attacks where an attacker could overwrite arbitrary files.
**Learning:** Writing predictable temporary files to world-writable directories like `/tmp` can lead to arbitrary file overwrite via symlink attacks.
**Prevention:** Use user-owned directories (e.g., via `os.homedir()` in Node or `$HOME` in bash) or dynamically generated temporary files (`mkstemp`/`mkdtemp`).
