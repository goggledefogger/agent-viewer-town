...
## 2026-05-25 - [Block "null" Origin and Fallback Middleware for CORS]
**Vulnerability:** Allowed origin validation permitted the string "null" because it throws an exception when passed to `new URL()`. The exception logic in `isAllowedOrigin` erroneously defaulted to `false` in `try/catch` which correctly blocked invalid origins, but allowed `cors()` middleware with `callback(null, false)` to fail open for simple requests (i.e. CORS blocks only work for preflight or client-side reading, but does not prevent the server from processing the request).
**Learning:** `cors()` middleware simply omitting headers when an origin is not allowed does not actively block simple requests from being executed on the server. Also, "null" is sent as origin for local files or sandboxed iframes and must be explicitly handled.
**Prevention:** Implement explicit string checking for `"null"` in origin validation. Additionally, place a custom fallback middleware after `cors()` which actively returns `403 Forbidden` if the `req.headers.origin` does not match allowed domains.
