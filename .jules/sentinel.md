...
## 2025-02-27 - Express CORS Middleware Bypass
**Vulnerability:** The application used `cors({ origin: ... callback(null, false) })` for invalid origins without a fallback middleware. This only omitted CORS headers on responses to unauthorized origins, meaning cross-origin requests still reached state-modifying endpoints, allowing for Cross-Site Request Forgery (CSRF) side effects. Also, sandboxed iframes can bypass simple CORS checks if 'null' origin is not strictly rejected.
**Learning:** `callback(null, false)` in the express `cors()` middleware does not block execution of the rest of the route handlers. It simply instructs the middleware not to set CORS response headers.
**Prevention:** Always implement an explicit fallback middleware right after `cors()` to intercept and return a `403 Forbidden` if `req.headers.origin` does not meet origin validation requirements. Explicitly block the string `'null'` during origin validation to guard against sandboxed iframe bypasses.
