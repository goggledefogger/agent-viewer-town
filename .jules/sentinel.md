...
## $(date +%Y-%m-%d) - Prevent Predictable Temp File Symlink Attack in Touch Bar Integration
**Vulnerability:** The Touch Bar integration in `packages/server/src/touchbar.ts` was writing a JSON status file to a predictable path in a world-writable directory (`/tmp/agent-viewer-touchbar.json`). This exposed the application to symlink attacks, where an attacker could create a symbolic link at that path pointing to a sensitive file, causing the application to overwrite it.
**Learning:** Hardcoded temporary files in world-writable directories like `/tmp` are a common security pitfall. This issue is particularly dangerous when the application runs with elevated privileges or handles sensitive data.
**Prevention:** Always write temporary files to a secure, user-owned directory (e.g., using `os.homedir()`) or dynamically generate temporary files using unique identifiers (e.g., `fs.mkstemp` or `fs.mkdtemp`).
