...
## 2025-07-09 - [Fix Timing Attack in Token Validation]
**Vulnerability:** The token validation in `auth.ts` used strict equality (`===`) to compare the user-provided token with the server's configured `AUTH_TOKEN`. This allows an attacker to leak the token character by character via a timing attack because the string comparison stops at the first mismatched character.
**Learning:** String equality (`===`) is unsafe for comparing secrets, such as authentication tokens or passwords, as V8/Node.js optimizes string comparisons. A short-circuit comparison takes a measurably different amount of time depending on how many characters matched.
**Prevention:** Always hash both the expected secret and the provided token, then use a constant-time comparison function like `crypto.timingSafeEqual` from the `crypto` module to safely compare the hashed tokens. Using a hash before comparison mitigates length leakage.
